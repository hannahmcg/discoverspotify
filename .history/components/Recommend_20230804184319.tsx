    ...
    <div className="items-center space-y-5 p-5">
      ...
      <div className="flex flex-wrap space-x-10">
        ...
        <h3>Based off your top artists</h3>
          ...
          <div className="flex flex-col space-y-2 bg-slate-800 mt-5 p-3 rounded-lg h-[500px] w-[500px] overflow-y-scroll overflow-x-scroll">
            {playlist2?.map((track) => (
              <div key={track.id} className="flex space-x-5 items-center ">
                <Image src={track.album?.images?.[0]?.url} alt="" width={30} height={30}/>
                <p>{track.artists?.[0]?.name}:</p>
                <p>{track.name}</p>
              </div>
            ))}
